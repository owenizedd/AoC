const lines = `Game 1: 4 red, 1 green, 15 blue; 6 green, 2 red, 10 blue; 7 blue, 6 green, 4 red; 12 blue, 10 green, 3 red
...
Game 100: 11 red, 1 blue, 2 green; 3 red, 3 green; 1 blue, 8 red, 4 green; 5 green, 5 blue, 1 red; 2 green, 1 red, 6 blue; 2 green, 8 red, 1 blue`.split('\n');
...
for (const i in lines) {
  ...
  for (const set of sets) {
    ...
    for (const item of set) {
      if (item.includes('red')) r += +item.split(' ')[0];
      if (item.includes('green')) g += +item.split(' ')[0];
      if (item.includes('blue')) b += +item.split(' ')[0];
    }
    if (r <= 12 && g <= 13 && b <= 14) setSafe++;
  }
  if (setSafe === sets.length) ans += +i + 1;
}
console.log(ans)