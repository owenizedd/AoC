...
import { count2DArray, generate2DArray, loopThroughDirections, p, print2DArray } from "../helper.js";
import { setIntersect } from "mathjs";
import { setUnion } from "mathjs";

let lines2 = `L.F--FF|.-.FF-777.F7FL-F.7.F-77.F--FF--.JJ7..F-77|FFF|7-F-J7--F7|7-F7FJ7-F7FF--7--7F-|7F--.|FF|7FF-F77-F7FJ777.FJ.FJ7-|-7-FFFJFF-77L7LF|FL77
...
.-J7LFJL-.|F----JLJLJF7F-7|F7JF7JLJFJ|FJ||LJ|L-7|F-7F-7F7F7F-7F--J7LJFJL7JL-7F7|||L--7FJJF-JF7|||||||||F--------JLJFJF---J||L7|FJ|F777-FF.F|
...
|-7-JJJ7J-LFJLLLF--FJF|-JJF|JLJJ-|-LJJ--LJL|-L--JJL-JF--J.L...LL.|JLF-L.-LF-J-7JJF.F.LLFJ.-J-L-FL|JJLJ.FL--JL-LJ.7-J-LJ-77LL-.FJ.-7-JJ|-|J-J`.split('\n');
let lines = `..........
.F------7.
.|F----7|.
.||....||.
...
const ROW_LEN = lines.length;
const COL_LEN = lines[0].length;
const stepsGrid = generate2DArray(ROW_LEN + 1, COL_LEN + 1, 0);
stepsGrid[1][1] = 1;
...
const isLoop = (x, y) => {
  ...
  const leftTravelled = stepsGrid[x][y - 1];
  const rightTravelled = stepsGrid[x][y + 1];
  const upTravelled = stepsGrid[x - 1] ? stepsGrid[x - 1][y] : false;
  const downTravelled = stepsGrid[x + 1][y];

  if (c === '7') {
    return leftTravelled && downTravelled;
  }
  if (c === 'F') {
    return rightTravelled && downTravelled;
  }
  if (c === '|') {
    return upTravelled && downTravelled;
  }
  if (c === '-') {
    return leftTravelled && rightTravelled;
  }
  if (c === 'L') {
    return upTravelled && rightTravelled;
  }
  if (c === 'J') {
    return upTravelled && leftTravelled;
  }
...
const getOutsideCount = () => {
  ...
  for (const row in lines) {
    ...
    let facingUp = null;
    p(lines[row])
    for (const col in lines[row]) {
      
      const c = lines[row][col];
      if (c === "|"){
        console.assert(facingUp === null, "facingUp should be null");
        inside != inside;
      } else if (c === '-') {
        console.assert(facingUp !== null, "facingUp should not be null");
...
const solve = () => {
  ...
  while (q.length) {
    ...
    stepsGrid[x][y] = step ? step : 1;

    if (isLoop(x,y)) {
      
      setTimeout(() => {
        print2DArray(stepsGrid, false, '  ')
        const borderSets = new Set();
        for (const row in stepsGrid) {
          for (const col in stepsGrid[row]) {
            const c = stepsGrid[row][col];
            if (c) {
              borderSets.add(`${row},${col}`);
            }
          }
        }
        p([...getOutsideCount()].length)
        const outsides = setUnion([...getOutsideCount()], [...borderSets]);
        // p(outsides);
        p((ROW_LEN * COL_LEN) - outsides.length);
     }, 1000);
     break;
    }
    if (c === '7') {
