...
for (const line of lines) {
  ...
  const numberFirst = text.findIndex((val) => isNaN(+val) === false)

  const numberLast = text.reverse().findIndex((val) => isNaN(+val) === false)

  const newNumber = `${line[numberFirst]}${line[text.length - numberLast - 1]}`;
